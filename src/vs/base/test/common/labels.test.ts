...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'Kronos Code', separator: { label: ' - ' } }), 'Kronos Code');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'Kronos Code', separator: { label: ' - ' } }), 'Untitled-1 - Kronos Code');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'Kronos Code', separator: { label: ' - ' } }), 'monaco - Kronos Code');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Kronos Code', separator: { label: ' - ' } }), 'somefile.txt - monaco - Kronos Code');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'Kronos Code', separator: { label: ' - ' } }), '* somefile.txt - monaco - Kronos Code');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'Kronos Code', separator: { label: ' - ' } }), 'monaco - Kronos Code');
	});
